...
def lambda_handler(event, context):
    ...
    regions = json.dumps(
        api_session.get_country_geographic_regions(fa.COUNTRY_ID_AUS))

    # Just store BNE sites for now
    site_details = json.dumps(api_session.get_full_site_details(
    ...
    client.put_object(Body=fuel_brands, Bucket=bucket,
                      Key=create_reference_object("brands"))
    client.put_object(Body=fuel_types, Bucket=bucket,
                      Key=create_reference_object("fueltypes"))
    client.put_object(Body=regions, Bucket=bucket,
                      Key=create_reference_object("regions"))
    client.put_object(Body=site_details, Bucket=bucket,
                      Key=create_reference_object("sitedetails"))
